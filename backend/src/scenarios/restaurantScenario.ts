import type { ScenarioData, ScenarioNode } from "./cafeScenario.js";
...
function formatRestaurantLine(line: RestaurantLine): string {
  return `${line.speaker}: ${line.korean} / ${line.romanization ?? ""} / ${line.french}`;
}

function formatMiniDialogues(): string {
  return restaurantDialogueData.miniDialogues
    .map((dialogue) => {
      const lines = dialogue.lines
        .map((line) => `${line.speaker}: ${line.korean} / ${line.french}`)
        .join("\n");
...
export const restaurantScenario: ScenarioData = {
  id: "restaurant",
  title: restaurantDialogueData.scenarioTitle,
  description: restaurantDialogueData.scenarioDescription,
  context: `
L'utilisateur pratique une situation réaliste dans un restaurant BBQ à Séoul.
Le but est de savoir :

* commander du samgyeopsal ou du galbi
* demander une recommandation
* demander au personnel de griller la viande ou dire qu'on la grille soi-même
* commander un doenjang jjigae ou des œufs vapeur
* choisir le niveau d'épice
* demander plus de salade ou d'accompagnements
* payer par carte ou en espèces
* demander ou refuser le reçu
  `.trim(),
  learningGoals: [
    "Commander dans un restaurant BBQ coréen",
    "Demander une recommandation",
    "Gérer la cuisson de la viande",
    "Commander des accompagnements",
    "Exprimer le niveau d'épice souhaité",
    "Demander plus de salade ou de banchan",
    "Payer et répondre pour le reçu",
  ],
  pedagogical: {
    startNodeId: restaurantDialogueData.pedagogical.startNodeId,
    nodes: restaurantDialogueData.pedagogical.nodes as unknown as Record<
      string,
      ScenarioNode
    >,
  },
...
export function getRestaurantScenarioForPrompt(): string {
  const pedagogicalNodes = Object.values(restaurantScenario.pedagogical.nodes)
    .map(formatScenarioNode)
    .filter(Boolean)
    .join("\n");

  const realNodes = Object.values(restaurantScenario.real.nodes)
    .map(formatScenarioNode)
    .filter(Boolean)
    .join("\n");

  const serverLines = restaurantDialogueData.serverLines
    .map(formatRestaurantLine)
    .join("\n");

  const clientLines = restaurantDialogueData.clientLines
    .map(formatRestaurantLine)
    .join("\n");

  return `
SCÉNARIO RESTAURANT :
${restaurantScenario.context}

SITUATION :
${restaurantDialogueData.scenarioDescription}

OBJECTIFS D'APPRENTISSAGE :
${restaurantScenario.learningGoals.map((goal) => `- ${goal}`).join("\n")}
...
Tu joues principalement un serveur ou une serveuse dans un restaurant BBQ coréen.
Si l'utilisateur parle de restaurant, de commande, de viande, de cuisson, d'accompagnements, de paiement ou de reçu, réponds avec les informations du script restaurant.
Si l'utilisateur pose une question, réponds brièvement comme prof de coréen, puis reviens immédiatement à la scène si c'est pertinent.
Si l'utilisateur fait une erreur, corrige doucement, donne la phrase naturelle, puis continue la scène.
Si l'utilisateur est bloqué, propose 2 ou 3 réponses possibles tirées du script.
Ne fais pas de longue leçon.
Garde les réponses courtes pour un avatar vidéo.
`.trim();
}

export const restaurantScenarioContext = getRestaurantScenarioForPrompt();