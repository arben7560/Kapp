import type { ScenarioChoice, ScenarioData, ScenarioNode } from "./cafeScenario.js";
...
function buildMetroNodes(steps: MetroStep[]): Record<string, ScenarioNode> {
  ...
  for (const step of steps) {
    ...
    if (step.id === "start" && hasChoices && step.choices) {
      nodes[step.id] = {
        ...
        choices: step.choices.map(formatMetroChoice),
      };
    ...
    nodes[step.id] = {
      ...
      french: step.french || step.text,
      romanization: step.romanization,
      nextNodeId: hasChoices ? choiceNodeId : null,
    };
...
function formatMetroChoice(choice: MetroChoice): ScenarioChoice {
  return {
    id: choice.id,
    label: choice.label,
    korean: choice.korean || choice.label,
...
const metroNodes = buildMetroNodes(hongikToGangnamLesson.steps);

export const metroScenario: ScenarioData = {
  id: "metro",
  title: hongikToGangnamLesson.title,
  description: hongikToGangnamLesson.situation,
  context: `
L'utilisateur pratique une situation réaliste dans le métro de Séoul.
Il se trouve à Hongik University station et veut aller à Gangnam.
Le but est de savoir :

* demander son chemin vers une station
* comprendre quelle ligne prendre
* demander le quai et la direction
* demander la durée du trajet et le nombre d'arrêts
* vérifier s'il y a une correspondance
* demander quelle sortie prendre à l'arrivée
  `.trim(),
  learningGoals: [
    "Demander son chemin dans le métro",
    "Comprendre la ligne et la direction",
    "Demander le quai exact",
    "Comprendre la durée et les arrêts",
    "Demander s'il y a une correspondance",
    "Demander la bonne sortie à Gangnam",
  ],
  pedagogical: {
    startNodeId: hongikToGangnamLesson.steps[0]?.id || "start",
    nodes: metroNodes,
  },
  real: {
    startNodeId: hongikToGangnamLesson.steps[0]?.id || "start",
...
export function getMetroScenarioForPrompt(): string {
  const pedagogicalNodes = Object.values(metroScenario.pedagogical.nodes)
    .map(formatScenarioNode)
    .filter(Boolean)
    .join("\n");

  const realNodes = Object.values(metroScenario.real.nodes)
    .map(formatScenarioNode)
    .filter(Boolean)
    .join("\n");
  ...
  return `
SCÉNARIO MÉTRO :
${metroScenario.context}

SITUATION :
${hongikToGangnamLesson.situation}

OBJECTIF :
${hongikToGangnamLesson.objective}

OBJECTIFS D'APPRENTISSAGE :
${metroScenario.learningGoals.map((goal) => `- ${goal}`).join("\n")}
...
Tu joues principalement un passant coréen dans le métro de Séoul.
Si l'utilisateur demande son chemin, réponds avec les informations du script Hongik University vers Gangnam.
...
export const metroScenarioContext = getMetroScenarioForPrompt();
