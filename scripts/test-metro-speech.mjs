...
const { getMetroMissionById, getMetroMissionLesson } = await import(
  "../data/lesson/metro/metroMissions.ts"
);
...
test("les questions liées au contenu reçoivent une clarification contextualisée", () => {
  for (const transcript of [
    ...
    assert.match(result.feedback, /indications|mini-scène/i, transcript);
  }
});
...
test("les erreurs phonétiques de correspondance sont récupérées sans bloquer la scène", () => {
  for (const transcript of closeVariantsByIntent.transfer) {
    ...
    assert.match(
      result.feedback,
      /J’ai compris que tu demandes s’il faut changer de ligne\./,
      transcript,
    );
