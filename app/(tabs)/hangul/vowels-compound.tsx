...
const SCENES: Scene[] = [
  {
    ...
    curiosityHook:
      "Les voyelles composées ne sont pas de nouvelles lettres au hasard : ce sont des fusions logiques.",
    instruction:
  ...
  },
  {
    ...
    curiosityHook:
      "Ces sons demandent un mouvement : les lèvres s’arrondissent puis glissent vers la voyelle finale.",
  ...
  },
  {
    ...
    curiosityHook:
      "À ce stade, tu ne décodes plus seulement des formes : tu reconnais des mots coréens réels.",
    instruction:
      "Touche chaque mot pour entendre comment la voyelle composée vit dans un vrai mot.",
    teaser:
...
const buildFusionQuestion = (
  ...
  return {
    id: `fusion-${exp.id}`,
    expression: exp,
    prompt: "Quelle fusion crée cette voyelle ?",
...
const buildWindQuestion = (
  ...
  return {
    id: `wind-${exp.id}`,
    expression: exp,
    prompt: "Quel son glissé correspond à cette voyelle ?",
    options: shuffle([correct, ...distractors]).slice(0, 4),
    correctAnswer: correct,
    questionType: "wind",
  };
...
const generateQuiz = (scene: Scene): QuizQuestion[] => {
  if (scene.id === "fusion") {
    return scene.expressions.map((exp) =>
      buildFusionQuestion(exp, scene.expressions),
    );
  }

  if (scene.id === "wind-sounds") {
    return scene.expressions.map((exp) =>
      buildWindQuestion(exp, scene.expressions),
    );
  }

  if (scene.id === "vocabulary") {
    return scene.expressions.map((exp) =>
...
const getQuizTitle = (question?: QuizQuestion) => {
  ...
  if (question.questionType === "fusion") return "DÉFI DE FUSION";
  if (question.questionType === "wind") return "DÉFI DU SON W";
