export type MetroChoice = {
  id: string;
  label: string; // affiché dans les ChoiceChips (FR)
  nextId: string;

  // Données utiles pour plus tard si tu veux afficher / jouer la réplique utilisateur
  korean?: string;
  romanization?: string;
};
...
export type MetroStep = {
  id: string;
  speaker: "ai" | "user";
  text: string; // actuellement utilisé par metroIA.tsx

  // Données supplémentaires pour évolution future
  korean?: string;
  french?: string;
  romanization?: string;
  narrator?: string;
  phase?: "Accueil" | "Ligne" | "Direction" | "Trajet" | "Sortie" | "Fin";
...
export const metroLesson: MetroLesson = {
  id: "metro_hongik_to_gangnam",
  title: "Aller à Gangnam depuis Hongik University",
  situation:
    "Vous êtes à la station Hongik University (홍익대입구역). Vous voulez aller à Gangnam et vous demandez votre chemin à un passant coréen.",
  objective:
...
export function getMetroStepById(stepId: string): MetroStep | undefined {
  return metroLesson.steps.find((step) => step.id === stepId);
}

export function createInitialMetroState(): MetroState {
  const firstStep = getMetroStepById("start");

  return {
    lessonId: metroLesson.id,
    currentStepId: "start",
...
export function getNextMetroState(
  ...
  const nextStep = getMetroStepById(nextStepId);
