...
const HANGUL_CARD_BACKGROUND = "rgba(2,3,6,0.46)";
...
export function HangulLessonScreen({ moduleId }: { moduleId: string }) {
  ...
  React.useEffect(() => {
    ...
    const sceneIndex = module.scenes.findIndex(
      (scene) => scene.id === session.sceneId,
    );
    const sceneUnlocked =
      sceneIndex <= 0 ||
      !!saved.masteredScenes[module.scenes[sceneIndex - 1]?.id];
    if (sceneIndex < 0 || !sceneUnlocked || session.questions.length === 0)
      return;

    const restoredSession = restoreHangulQuizSession(
      session,
      module.scenes[sceneIndex].questions,
    );
    ...
    updateHangulProgress((current) => ({
      ...
      lessons: {
        ...current.lessons,
        [module.id]: {
          ...normalizeLesson(current.lessons[module.id]),
  ...
  if (!moduleUnlocked && prerequisite) {
    return (
      <SafeAreaView style={styles.safe}>
        ...
        >
          <LinearGradient
            colors={["rgba(2,3,6,0.72)", "rgba(2,3,6,0.96)"]}
          ...
          >
            <BlurView intensity={65} tint="dark" style={styles.introCard}>
              <AppText variant="sectionLabel" style={{ color: module.accent }}>
                PROGRESSION GUIDÉE
              </AppText>
              ...
              >
                <LinearGradient
                  colors={[prerequisite.accent, module.accent]}
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      >
        <LinearGradient
          colors={["rgba(2,3,6,0.62)", "rgba(2,3,6,0.88)"]}
          style={StyleSheet.absoluteFillObject}
        />
        <ScrollView
        ...
        >
          <View style={[styles.frame, { maxWidth: responsive.maxWidth }]}>
            ...
            <BlurView intensity={55} tint="dark" style={styles.introCard}>
              ...
              >
                ...
                <View
                  style={[
                    styles.stateBadge,
                    { borderColor: `${activeScene.accent}88` },
                  ]}
                >
                  <AppText
                    variant="caption"
                  ...
                  >
                    {sceneMastered
                      ? "RÉUSSI"
                      : lesson.completedScenes[activeScene.id]
                        ? "TERMINÉ"
                        : "À COMMENCER"}
                  </AppText>
                </View>
                ...
                return (
                  ...
                  >
                    <BlurView
                      ...
                      style={[
                        styles.card,
                        discovered && { borderColor: activeScene.accent },
                      ]}
                    >
            ...
            >
              <LinearGradient
                colors={[activeScene.accent, module.accent]}
                style={styles.primaryGradient}
              >
                <AppText variant="button" style={styles.primaryText}>
                  {!canStartQuiz
            ...
            {allScenesMastered ? (
              ...
              >
                ...
                <AppText variant="button" style={{ color: module.accent }}>
              ...
              >
                {!quizComplete && currentQuestion ? (
                  ...
                  <View style={styles.result}>
                    <AppText
                      variant="sectionLabel"
                      style={{ color: result.mastered ? "#4ADE80" : "#FDE047" }}
                    >
...
const styles = StyleSheet.create({
  ...
  introCard: {
    ...
    backgroundColor: HANGUL_CARD_BACKGROUND,
    padding: 20,
    overflow: "hidden",
  },
  introHeader: {
